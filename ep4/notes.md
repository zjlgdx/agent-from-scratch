...
用过上千次 Claude Code 和 Cursor，调过很多模型，但过去如果被问到“工具调用的底层数据流是怎样的”“为什么模型不会直接编造工具结果”，其实心里是含糊的。亲手写完 EP0 到 EP3 这几百行内核代码之后，把当初最困惑的 10 个问题拿出来，用代码和实验现场答题。
...
**外层 catch 捕获的错误（API 400 / 网络报错）**：它也是在内层循环里发生的，`callModel` 抛出来，但 `runTurn` 里没有捕获它，一路抛到 REPL 那层包着整轮 `runTurn` 的 catch。到这里这轮对话已经进行不下去，而且没法把错误“告诉模型”——告诉它恰恰需要一次成功的 API 调用，而失败的正是 API 调用。唯一干净的出路是当这轮没发生过：单行 `messages.length = mark` 整轮回滚。否则半截孤儿消息会污染上下文，后续请求全部报废。API 失败是事务失败。回滚的只是消息历史，这轮已经落盘的文件不会跟着撤销，重试就可能把写操作再跑一遍，EP3 的 `replay` 标注就是冲这个来的。

### 问题 4：为什么没定义的工具模型也能编出来？调用次数到底怎么算？

工具调用不是 RPC，只是模型在预训练或微调里学会了按特定 JSON/XML 语法输出文本。真正的执行永远在宿主。

最硬的证据是 EP0 里的实测：删掉系统提示词里“没有联网能力”的约束后，用 `llama-3.3-70b-versatile` 问天气，模型凭空编出了一个从未定义过的 `brave_search` 调用，Groq 以 400 拒绝，`failed_generation` 字段里躺着那段格式标准的调用文本。而把约束加回去、换成 `gpt-oss-120b` 再问，它老实承认做不到。注意这两次提示词不一样，它说明的是“约束声明有没有”决定了模型是老实回答还是编工具，不是两个模型谁更老实：
...
新模型补一句：gpt-oss-120b、qwen3.8 没有 stop 也会自己停在 Action Input 后面，llama-3.3-70b 照样自导自演（见 EP1 笔记「新模型重测」）。stop 从唯一的截断手段退为最后保险，但这道闸门得留在宿主手里。
...
第二个是裁判权留在宿主，不给 Agent 传 bash。实测给了 `bash`，模型面对不熟悉的项目往往敲 `ls -R` 或全局 `grep`，整个 `node_modules` 灌进上下文，413。更重要的是分工：Agent 负责改代码，“改对了没有”由宿主主循环跑 `node target/app.ts` 来判，只认硬指标（FAIL 行数比修之前少没少），不让模型既当运动员又当裁判。

## 四、迈向生产运行宿主（Harness）—— 从玩具到工程护栏（Q7 ～ Q9）

### 问题 7：为什么不能让模型自己判断路径合不合规？

模型传 `../../etc/passwd`，裸代码直接读就穿透了。模型输出的参数都是不可信的外部输入，它可能被提示词注入带偏，也可能是幻觉，所以合规判断不能交给它，得由宿主在执行前做 `resolveSafePath` 规范化和沙箱校验。
...
代码跑通是其次，最大的收获是黑盒破了。往后再看 LangChain、Claude Code、Pi 这类框架，能一眼认出底下的骨架。
