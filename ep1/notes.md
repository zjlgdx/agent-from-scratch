...
如果手头只有一个普通的纯文本接口，如何让模型在发起工具调用时暂停生成、等本地代码执行完再继续推理？
...
// 2. stop: ["\nObservation:"] —— 核心截断机制，模型刚要输出工具结果即被服务端截断；
// 3. temperature: 0 —— 采用贪心解码，格式遵守率明显更高（可改回默认值观察差异）。
```
...
单次 API 响应里，模型连续输出了 `Thought`、`Action`、`Action Input`、`Observation` 直至 `Final Answer`，宿主未能介入执行，磁盘上也未生成 `hello.py`。缺少 `stop` 截断，ReAct 就会退化为模型自发模拟的纯文本输出。
...
随机性增加后，模型开始吐出不合规的标签（如 `Action: None`）；宿主把解析报错当成 `Observation` 回填给模型，模型在下一轮成功自愈，退回到 `Final Answer`。这暴露了 2022 年手工 ReAct 对格式遵守率有多敏感；该模型在默认温度下输出格式极易漂移，`temperature: 0` 属于必须保留的约束参数。
...
未提供联网或天气查询工具时，模型自主创建了 `weather.txt` 写入 `sunny`，随后再读取该文件作为答案输出。这说明给模型配置工具时，如果不明确限定能力边界，模型会尝试组合手头已有的工具来强行闭合逻辑。
...
gpt-oss-120b 和 qwen3.8 则表现不同：即使未配置 stop 序列，它们在输出 Action Input 后也会自行停下，等待宿主回填结果。模型较好地遵循了提示词中“Observation 一行由系统填入，你绝对不要自己写”的指令。此时 `stop` 不再是唯一的截断手段，但仍然是宿主必须持有的防线：不能将控制流完全押注于模型的自觉停顿；更换模型或服务商后行为随时可能漂移，Cloudflare 上的 llama 就印证了这一点。
...
这印证了下文机制对照表的结论：函数调用的交互模式已被固化进模型训练之中，单纯通过提示词指令难以逆转这一行为习惯。2022 年的手工 ReAct 在这个模型上跑不动，并非模型缺乏推理能力，而是其输出格式已强制与结构化工具调用深度绑定。
...
- 在缺乏原生协议支持时，`stop` 序列是打断模型连续生成、将控制权交还宿主的关键机制。即使较新的模型已能按指令主动停止，`stop` 依然是不可或缺的底层兜底护栏。
